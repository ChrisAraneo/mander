...
export function drawPortal(context: CanvasRenderingContext2D, state: GameState): void {
  ...
  const swirl = context.createRadialGradient(centerX, centerY, 2, centerX, centerY, portal.width / 2 + 8);
  swirl.addColorStop(0, '#e9dcff');
  swirl.addColorStop(0.45, '#8d55e0');
  swirl.addColorStop(1, '#3c2470');
  context.fillStyle = swirl;
  context.beginPath();
  context.ellipse(centerX, centerY, (portal.width / 2) * pulse, (portal.height / 2) * pulse, 0, 0, Math.PI * 2);
  context.fill();
