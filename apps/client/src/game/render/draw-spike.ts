...
export function drawSpike(context: CanvasRenderingContext2D, tileX: number, tileY: number): void {
  context.strokeStyle = '#333a4f';
  context.lineWidth = 1;
  for (const [left, apex, right] of spikeTriangles(tileX, tileY)) {
    const gradient = context.createLinearGradient(left.x, left.y, left.x, apex.y);
