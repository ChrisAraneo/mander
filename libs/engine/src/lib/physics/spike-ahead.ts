...
import { TILE_SIZE, isSpike, type Level } from '@mander/generator';
import { ENEMY_HEIGHT, ENEMY_WIDTH } from '../state';

export function spikeAhead(level: Level, originX: number, originY: number, facing: 1 | -1): boolean {
  ...
  for (let row = topRow; row <= bottomRow; row++) if (isSpike(level, column, row)) return true;
  return false;
}
