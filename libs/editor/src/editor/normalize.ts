...
import { AIR, BLOCK, ENEMY, SECTOR_WIDTH, STRUCTURE_HEIGHT, type Structure } from '@mander/generator';
...
export function normalize(data: unknown): Structure | null {
  ...
  if (rows.length < STRUCTURE_HEIGHT) {
    const padding = times(STRUCTURE_HEIGHT - rows.length, () => fill(new Array(SECTOR_WIDTH), AIR));
