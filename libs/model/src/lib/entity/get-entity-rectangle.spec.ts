...
import { getEntityRectangle } from './get-entity-rectangle';
...
describe('getEntityRectangle', () => {
  ...
  it('should place the box against the tile when the tile lies deep in the level', () => {
    expect(getEntityRectangle({ x: 4, y: 7 }, BOX)).toEqual({
      x: 4 * TILE_SIZE + 3,
      y: 8 * TILE_SIZE - 22,
      width: 26,
      height: 22,
  ...
  it('should carry the width and height straight through when given a box of its own', () => {
    const rectangle = getEntityRectangle({ x: 2, y: 2 }, PORTAL_ENTITY_BOX);
  ...
  it('should shift by a whole tile when the tile steps one across', () => {
    expect(getEntityRectangle({ x: 1, y: 0 }, BOX).x).toBe(
      getEntityRectangle({ x: 0, y: 0 }, BOX).x + TILE_SIZE,
    );
  });

  it('should shift by a whole tile when the tile steps one down', () => {
    expect(getEntityRectangle({ x: 0, y: 1 }, BOX).y).toBe(
      getEntityRectangle({ x: 0, y: 0 }, BOX).y + TILE_SIZE,
    );
  });
  ...
  it('should stand the box flat on the floor of its tile when it belongs to the chest or the portal', () => {
    forEach([CHEST_ENTITY_BOX, PORTAL_ENTITY_BOX], (box) => {
      const rectangle = getEntityRectangle({ x: 5, y: 6 }, box);
