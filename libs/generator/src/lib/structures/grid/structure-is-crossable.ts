...
export function structureIsCrossable(grid: Structure): boolean {
  ...
  const exitIndex = findIndex(surfaces, (surface) => surface.col === SECTOR_WIDTH - 1);
  if (entryIndex < 0 || exitIndex < 0) return false;
  return reachable(surfaces, entryIndex).has(exitIndex) && reachable(surfaces, exitIndex).has(entryIndex);
}
