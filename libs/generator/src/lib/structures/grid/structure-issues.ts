...
export function structureIssues(grid: Structure): string[] {
  ...
  if (!every(grid, (row) => every(row, (cell) => cell === AIR || cell === BLOCK || cell === ENEMY))) {
    issues.push('cells must be 0 (air), 1 (block) or 2 (enemy)');
  }
  if (issues.length > 0) return issues;

  if (groundHeight(grid, 0) !== 1) {
    issues.push('the left edge must be flush ground: only the bottom-left cell a block');
  }
  if (groundHeight(grid, SECTOR_WIDTH - 1) < 1) {
    issues.push('the right edge must be solid ground so the next structure connects');
  }
  ...
  if (!enemiesHaveFooting(grid)) {
    issues.push('every enemy (2) needs a block directly beneath it to stand on');
  }
  return issues;
}
