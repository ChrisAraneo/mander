import { TILE_DIRT, type Tile } from '@mander/model';
import { map, range } from 'lodash-es';
import { describe, expect, it } from 'vitest';

import { sortHorizontalPlayerSpawnCandidates } from './sort-horizontal-player-spawn-candidates';

const LEVEL: Tile[][] = [[TILE_DIRT]];

const sorted = (columns: number[]): number[] =>
  map(
    sortHorizontalPlayerSpawnCandidates({
      tiles: LEVEL,
      candidates: map(columns, (column) => ({ row: 3, column })),
    }).candidates,
    'column',
  );

describe('sortHorizontalPlayerSpawnCandidates', () => {
  it('should put column 1 first when all columns are free', () => {
    expect(sorted(range(6))[0]).toBe(1);
  });

  it('should put the columns near the left edge first when the level is six wide', () => {
    expect(sorted(range(6))).toEqual([1, 2, 3, 0, 4, 5]);
  });

  it('should put the far columns last when the level is wider than six', () => {
    expect(sorted(range(8))).toEqual([1, 2, 3, 0, 4, 5, 6, 7]);
  });

  it('should sort the far columns from left to right when it only gets far columns', () => {
    expect(sorted([8, 6, 7])).toEqual([6, 7, 8]);
  });

  it('should sort only the columns it gets when some are missing', () => {
    expect(sorted([5, 3, 0])).toEqual([3, 0, 5]);
  });

  it('should keep the row of each candidate when it sorts', () => {
    expect(
      sortHorizontalPlayerSpawnCandidates({
        tiles: LEVEL,
        candidates: [
          { row: 4, column: 3 },
          { row: 2, column: 1 },
        ],
      }).candidates,
    ).toEqual([
      { row: 2, column: 1 },
      { row: 4, column: 3 },
    ]);
  });

  it('should give nothing back when there are no candidates', () => {
    expect(sorted([])).toEqual([]);
  });

  it('should keep the grid the same when it sorts', () => {
    expect(
      sortHorizontalPlayerSpawnCandidates({ tiles: LEVEL, candidates: [] })
        .tiles,
    ).toBe(LEVEL);
  });
});
