...
import { createPortalCandidates } from './create-portal-candidates';
...
// columns 0 and 1 can hold a portal, column 2 has no floor under it
const LEVEL = grid(['...', '...', '##.']);

describe('createPortalCandidates', () => {
  it('should give the rows above the floor when the column has space', () => {
    expect(
      createPortalCandidates({ tiles: LEVEL, columns: [0] }).candidates,
    ).toEqual([{ column: 0, rows: [1, 0] }]);
  });

  it('should give no rows when the column has no space', () => {
    expect(
      createPortalCandidates({ tiles: LEVEL, columns: [2] }).candidates,
  ...
  it('should keep the columns in the same order when it makes the list', () => {
    expect(
      map(
        createPortalCandidates({ tiles: LEVEL, columns: [2, 0, 1] }).candidates,
        'column',
      ),
    ).toEqual([2, 0, 1]);
  });
  ...
  it('should make one item per column when it gets many columns', () => {
    expect(
      createPortalCandidates({ tiles: LEVEL, columns: [0, 1, 2] }).candidates,
    ).toHaveLength(3);
  });

  it('should make an empty list when there are no columns', () => {
    expect(
      createPortalCandidates({ tiles: LEVEL, columns: [] }).candidates,
    ).toEqual([]);
  });

  it('should keep the grid the same when it makes the list', () => {
    expect(createPortalCandidates({ tiles: LEVEL, columns: [] }).tiles).toBe(
      LEVEL,
    );
  });
});
