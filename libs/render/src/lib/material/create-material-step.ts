...
import { formatHslCss, parseHsl, shiftHsl } from '@mander/utils';
...
const createBrickStep = (
  pixelX: number,
  pixelY: number,
  style: MaterialStyle,
): CanvasStep =>
  sequence([
    applyStyle({ fillStyle: style.joint }),
    sequence(
...
const createStoneStep = (
  ...
  sequence([
    ...
    applyStyle({ fillStyle: style.highlight }),
...
const getPlankTones = memoize((base: string): string[] =>
  match(parseHsl(base))
    .with(undefined, () => map(PLANK_LIGHTNESS, () => base))
    .otherwise((color) =>
      map(PLANK_LIGHTNESS, (lightness) =>
        formatHslCss(shiftHsl(color, { lightness })),
      ),
    ),
);

const hashTileNoise = (a: number, b: number): number =>
  (((a * 73856093) ^ (b * 19349663)) >>> 0) % 9973;

const createPlankStep = (
  pixelX: number,
  top: number,
  seed: number,
  style: MaterialStyle,
): CanvasStep =>
  chain(getPlankTones(style.base))
    .thru((tones) => ({
    ...
    .thru(({ tone, buttX }) =>
      sequence([
        ...
        applyStyle({ fillStyle: GRAIN_LIGHT }),
    ...
    .thru(({ column, row }) =>
      map(range(2), (course) =>
        createPlankStep(
          ...
          hashTileNoise(column, row * 2 + course),
          style,
        ),
      ),
    )
...
export const createMaterialStep = (
  tile: Tile,
  pixelX: number,
  pixelY: number,
  style: MaterialStyle,
  ...
  match(tile)
    .with(TILE_BRICK, () => createBrickStep(pixelX, pixelY, style))
    .with(TILE_STONE, () => createStoneStep(pixelX, pixelY, style))
    .with(TILE_WOOD, () => createWoodStep(pixelX, pixelY, style))
    .with(TILE_CERAMIC, () => createCeramicStep(pixelX, pixelY, style))
    .with(TILE_FIREBALL, () => createFireballTileStep(pixelX, pixelY, style))
    .otherwise(() => skip);
